...
cp "$ROOT/tests/splitscreen/phase5/three-player/combat.cfg" "$HOMEPATH/base/splitqa/phase5_3p_combat.cfg"
...
OPENJK_VIRTUAL_GAMEPADS=3 OPENJK_VIRTUAL_GAMEPAD_PORT="$PORT" "$BIN" \
	...
	+set vm_game 0 +set vm_cgame 0 +set vm_ui 0 +set net_port 29193 \
	+set r_fullscreen 0 +set s_initsound 0 +set in_joystick 1 +set developer 1 \
	+exec splitqa/phase5_3p_combat.cfg >"$LOG" 2>&1 &
game_pid=$!
...
send_once() {
	local marker=$1
	shift
	if rg -q "Phase5ThreeCombat: $marker" "$LOG" 2>/dev/null && [[ -z ${sent[$marker]+x} ]]; then
...
for _ in $(seq 1 2600); do
	...
	send_once READY_P3_KILL_P1 gamepad 2 button 0 down wait 6000 gamepad 2 button 0 up
	...
	send_once READY_ALL_FORCE gamepad 1 button 10 tap gamepad 2 button 10 tap wait 500 gamepad 1 button 10 tap gamepad 2 button 10 tap
...
wait "$game_pid"
trap - EXIT
for marker in READY_P1_KILL_P2 READY_RESPAWN_P2 READY_P2_KILL_P3 READY_RESPAWN_P3 READY_P3_KILL_P1 READY_RESPAWN_P1 READY_ALL_ATTACK READY_ALL_FORCE; do
	rg -q "Phase5ThreeCombat: $marker" "$LOG"
done
if rg -q "Split(NetLifecycle|NetStat)Assert: FAIL|SplitInputAssertCmd: FAIL|SplitUIAssert: FAIL|SplitNetStagePair: FAIL" "$LOG"; then
	rg "Split(NetLifecycle|NetStat|Input)Assert|SplitUIAssert|SplitNetStagePair|Phase5ThreeCombat:" "$LOG"
	exit 1
fi
[[ $(rg -c "SplitNetLifecycleAssert: PASS" "$LOG") -ge 12 ]]
echo "three-player combat QA passed"
