...
python3 "$root/tests/splitscreen/gameplay/certification/generate_asymmetric_recovery_cfg.py" \
	"$players" "127.0.0.1:$OPENJK_E2E_SERVER_PORT" \
	>"$home/base/splitqa/asymmetric_recovery.cfg"
...
"$bin" +set fs_basepath "$base" +set fs_homepath "$home" \
	...
	+set com_introplayed 1 +exec splitqa/asymmetric_recovery.cfg
find "$home/base/screenshots" -type f -name 'cert_asymmetric_*.png' \
