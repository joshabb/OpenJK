...
if [[ "$role" == host ]]; then
	home="$OPENJK_E2E_HOMEPATH"; cfg="audio_${players}p.cfg"; port="$OPENJK_E2E_CLIENT_PORT"
else
	home="$OPENJK_E2E_RUN_DIR/remote-audio-home"; cfg=remote.cfg; port="$OPENJK_E2E_REMOTE_PORT"
fi
mkdir -p "$home/base/splitqa" "$home/base/screenshots"
"$ROOT/tests/splitscreen/install_assets.sh" "$home" >/dev/null
cp "$ROOT/tests/splitscreen/gameplay/runtime/audio/cfg/$cfg" "$home/base/splitqa/"
...
"$bin" +set fs_basepath "$base" +set fs_homepath "$home" +set vm_game 0 \
	...
	+set r_mode 3 +set s_initsound 1 +set in_joystick 1 +set com_introplayed 1 \
	+set developer 1 +set gp305_connect "connect 127.0.0.1:$OPENJK_E2E_CLIENT_PORT" \
	+exec "splitqa/$cfg"
if [[ "$role" == host ]]; then
